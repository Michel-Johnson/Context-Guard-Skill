...
没有上一步/下一步按钮，也没有单独的跟练模式。小屏通过镜头取景展示操作；自由体验保留桌面工作台并允许滚动，不另外设计一套移动产品 UI。响应式实测状态见 `QA.md`。
...
独立工作流 `.github/workflows/site-pages.yml`：从 `codex/promotion-site` 构建并部署，`main` 和 PR 只构建。手动运行也仅在宣传分支部署。工作台源码与 `SKILL.md` 更新触发重建，发布前运行现有时序与镜头检查。部署 job 单独获得 Pages/OIDC 权限，构建 job 只有读取权限。

只上传 `site/dist`，不上传真实 `.codex/context`，不改变 npm 包、版本或发布链。Pages Source 使用 GitHub Actions，`github-pages` 环境只允许宣传分支部署。网站可先独立上线，不合并或修改受保护的 `main`；未来合入仍须遵循原 PR/Required 检查。
