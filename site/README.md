...
英文版覆盖四端对话、App 首用三步、接续与 Bug 场景、独立工作台示例、Debug、安装反馈和折叠说明。英文首用示例回答选择 English，随后展示 `--language en`。英文需求和回复按14ms/字符输入，中文24ms；调用命令仍为28ms/字符，完成整句后才发送或接续。
...
站点外层分为首页、工作台、App 调用、项目记忆、Debug 和安装六页。页面本身固定为当前视口，不使用长文档滚动；鼠标滚轮或触控板纵向手势一次翻一页，页头链接、`PageUp / PageDown`、上下方向键和 URL 共用同一页状态。连续滚轮事件会合并为一次手势，避免惯性跳过多页；输入框、文本框和选择器保留自身滚轮行为。页面不再额外显示底部进度线或翻页按钮，当前位置由 URL、页头和当前内容共同表达。

首页左侧只保留主张与两个主要入口，右侧直接缩放展示与当前提交同源生成的 1440 × 900 工作台画面。中英文页面分别加载对应语言的真实工作台文件，不维护独立截图或另一套仿制界面；点击画面进入独立工作台页。

`#workbench`、`#clients`、`#memory`、`#debug`、`#install` 仍是可分享的直达链接。翻页更新 URL 历史，浏览器前进/后退会恢复对应页面；切换中英文保留当前页。非当前页保留组件状态，但通过 `inert` 与 `aria-hidden` 移出交互和无障碍序列；客户端时钟在离屏时冻结，返回后继续。

工作台、客户端和 Debug 的展示宽度同时受视口高度约束，使产品画面和必要控制位于同一屏。桌面端把工作台功能、客户端/流程选择和 Debug 标题收进 142px 侧栏，把剩余宽度完整交给动画舞台；窄屏改用选择器并继续按原产品比例取景。客户端只展示 App 内的三个首用动作与两个后续场景，不挂载工作台 iframe；工作台内部的标签、缩放、拖动、自由体验和 iframe 操作全部位于下一整屏页。
...
“App 调用”包含 Codex App、Codex CLI、Cursor、Claude Code 四个独立入口，共享调用 Skill、确认语言、准备工作台三步，以及接续项目和反馈 Bug。实际工作台动画位于它之前的独立整屏页。安装位于下方独立区域。Codex App 已按同版本本机窗口截图修正可见结构；当前自动检查与人工验收缺口见 `QA.md`，不宣称逐像素还原。
...
`npm run test:camera` 检查真实画布尺寸、固定取景倍率、输入完成后的1秒停顿、较慢回复取景、连续移动、换章归零、暂停恢复、不同采样频率及全窗反向切换。这些是源码/数学回归，不代替真实页面的可读性与流畅度验收。
...
App 演示只保留一份导航：首次使用三步直接展开，接续工作和 Debug 在其后。第三步只在客户端对话里展示取得工作台入口，不在这个页面挂载或播放工作台。完整/单章播放控件归入同一导航，当前项随自动播放同步；从其他场景可直接跳到任一首次使用步骤。客户端侧栏不再重复展示“使用前提”等辅助说明，安装条件统一留在 README 与安装页。

桌面演示导航固定142px宽、与舞台间隔16px；四个客户端在侧栏顶部组成两列，首用章节与后续场景在下方连续排列。宽度不超过720px或高度不超过600px时，客户端和章节都改用选择框。外层只保留导航、当前步骤、必要按钮和错误反馈；官方使用条件留在 README 与安装页，演示边界保留在页面底部的一条说明中。

首次使用三步：调用 Skill → 确认语言 → 准备工作台。三轮客户端对话保留前文，最后给出进入独立工作台页的入口；建图、模块操作和记忆全部留在上一整屏的真实工作台演示。后续输入不重复调用命令，中文版选择中文、英文版选择 English，仅为示例，不代替实际用户选择。

App 完整播放自动接续；第三步结束前，既有指针会完成移动与点击，随后自动进入独立工作台页的“逐层建图”。单章播放仍在普通章节结束后暂停；若单独播放第三步，同样在点击入口后进入工作台。工作台自身的六个章节继续使用真实完成事件、原控件与 iframe，不与客户端播放器共享章节状态。减少动态时显示当前步骤完成状态，用章节按钮继续。

“查看全窗 / 聚焦操作”只切换视角，不改变播放状态或进度：正在播放时继续打字并接续章节，已经暂停时仍保持暂停。两种状态都保留柔和镜头过渡，不需要因为查看全窗而重新点击播放。

客户端页不创建工作台实例，因此工作台的载入、暂停或退场事件不会改变 App 动画。排查预览问题时使用带当前构建参数的页面链接完整重载；同一地址仅变化 `#clients` 不保证重新加载脚本。资源健康检查需同时核对状态码与MIME，避免将缺失资源的HTML回退误认为有效JS。

步骤切换不重挂聊天组件，已完成消息保留稳定 key。滚动按时间增量连续追随新增内容，不因每个字符重新启动补间。切换客户端保留各自步骤和播放意图，隐藏端冻结；翻到工作台时由整屏轨道完成过渡。

命令从首字符输入：调用命令28ms/字符，中文需求和回复24ms/字符、英文14ms/字符；CLI的启动命令也逐字出现。时序按客户端实际调用文本长度计算，发送前输入必须完整。镜头、菜单、工具记录和结果入口共用一套时序。

输入结束后180ms发送；从最后一个字符算起，镜头保持输入区1秒，再以较慢速度移向回复区，首条工具记录在镜头启动600ms后出现。工具记录间隔180ms，回复结束后保留700–1200ms阅读时间。工作台普通步骤450ms、写入后800ms；产品1600ms Bug原型计时保留。总时长随语言和输入长度变化，以完成事件接续，不用固定时长跳过尚未结束的动作。
...
- 首次使用：App 页先展示语言提问，再逐字播放示例用户的语言回复、设置记录语言和请求打开工作台。下一整屏的真实工作台独立展示逐层建图、模块记忆等操作；不提前生成第二层，不宣称已经保存到用户项目磁盘。
...
演示播放器没有重复的外层跟练按钮；站点页面通过滚轮、键盘或页头直达切换。导览模式下，内嵌工作台会把滚轮交给外层翻页；进入“亲自试试”或直接操作工作台后，滚轮归还真实画布。小屏通过镜头取景展示操作；自由体验保留桌面工作台并允许在产品画布内部滚动，不另外设计一套移动产品 UI。响应式实测状态见 `QA.md`。
...
- `src/UsageNavigation.tsx`：连续故事和独立片段共用唯一的流程导航；三个 App 步骤、后续场景和播放方式集中在同一处，步骤状态仍由客户端播放器持有。
