...
站点外层分为首页、工作台、App 调用、项目记忆、Debug 和安装六页。页面本身固定为当前视口，不使用长文档滚动；鼠标滚轮或触控板纵向手势一次翻一页，页头链接、`PageUp / PageDown`、上下方向键和 URL 共用同一页状态。连续滚轮事件会合并为一次手势，避免惯性跳过多页；输入框、文本框和选择器保留自身滚轮行为。底部只显示不可点击的进度线，不放页面翻页按钮。
...
“App 调用”包含 Codex App、Codex CLI、Cursor、Claude Code 四个独立入口，共享调用 Skill、确认语言、准备工作台三步，以及接续项目和反馈 Bug。实际工作台动画位于它之前的独立整屏页。安装位于下方独立区域。Codex App 已按同版本本机窗口截图修正可见结构；本轮英文网页回放见 `QA.md`，未实拍原生状态与环境缺口见参考清单，不宣称逐像素还原。
...
App 演示只保留一份导航：首次使用三步直接展开，接续工作和 Debug 在其后。第三步只在客户端对话里展示取得工作台入口，不在这个页面挂载或播放工作台。完整/单章播放控件归入同一导航，当前项随自动播放同步；从其他场景可直接跳到任一首次使用步骤。

桌面演示导航固定142px宽、与舞台间隔16px；四个客户端在侧栏顶部组成两列，首用章节与后续场景在下方连续排列。宽度不超过720px或高度不超过600px时，客户端和章节都改用选择框。外层只保留导航、当前步骤、必要按钮和错误反馈；官方使用条件与演示边界放入折叠说明。
...
App 完整播放自动接续，到第三步停止；单章播放在本章结束后暂停。向上滚轮返回上一整屏工作台，或点击对话中的工作台入口直达“逐层建图”。工作台自身的六个章节继续使用真实完成事件、原控件与 iframe，不与客户端播放器共享章节状态。减少动态时显示当前步骤完成状态，用章节按钮继续。
...
- `src/UsageNavigation.tsx`：连续故事和独立片段共用唯一的流程导航；三个 App 步骤、后续场景、播放方式及使用前提集中在同一处，步骤状态仍由客户端播放器持有。
