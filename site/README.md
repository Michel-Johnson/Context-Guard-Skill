...
站点外层分为首页、工作台、客户端使用、项目记忆、Debug 和安装六页。页面本身固定为当前视口，不使用长文档滚动；鼠标滚轮或触控板纵向手势一次翻一页，页头链接、`PageUp / PageDown`、上下方向键和 URL 共用同一页状态。连续滚轮事件会合并为一次手势，避免惯性跳过多页；输入框、文本框和选择器保留自身滚轮行为。底部只显示不可点击的进度线，不放页面翻页按钮。
...
工作台、客户端和 Debug 的展示宽度同时受视口高度约束，使产品画面和必要控制位于同一屏。桌面端把工作台功能、客户端/流程选择和 Debug 标题收进 142px 侧栏，把剩余宽度完整交给动画舞台；窄屏改用选择器并继续按原产品比例取景。客户端的七章与两个后续场景都保留，工作台内部的标签、缩放、拖动、自由体验和 iframe 操作仍由原控件处理。
