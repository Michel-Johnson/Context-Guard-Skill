...
“设计问答”是独立整屏页面，用来解释产品决策，不作为通用帮助中心。每条内容由短标题、完整问题、一句话结论、原因、三步关系链和项目实例组成；访问者一次只阅读一个答案，避免把宣传页堆成密集 FAQ。

问题数据集中在 `src/design-questions.ts`，展示组件为 `src/DesignQuestions.tsx`。后续从开发过程补充内容时，只收录能够解释稳定产品方法的问题；临时排错、版本状态和操作说明仍留在 QA 或开发记录中。每条新增内容必须同时补齐英文词典，并保持问题与所属模块、决定或坏例之间的关系可说清楚。
