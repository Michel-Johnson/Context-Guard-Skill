...
站点外层分为首页、App 调用、工作台、项目记忆、Debug 和安装六页。页面本身固定为当前视口，不使用长文档滚动；鼠标滚轮或触控板纵向手势一次翻一页，页头链接、`PageUp / PageDown`、上下方向键和 URL 共用同一页状态。连续滚轮事件会合并为一次手势，避免惯性跳过多页；输入框、文本框和选择器保留自身滚轮行为。底部只显示不可点击的进度线，不放页面翻页按钮。
...
“App 调用”包含 Codex App、Codex CLI、Cursor、Claude Code 四个独立入口，共享调用 Skill、确认语言、准备工作台三步，以及接续项目和反馈 Bug。实际工作台动画位于相邻的独立整屏页。安装位于下方独立区域。Codex App 已按同版本本机窗口截图修正可见结构；本轮英文网页回放见 `QA.md`，未实拍原生状态与环境缺口见参考清单，不宣称逐像素还原。
...
首次使用三步：调用 Skill → 确认语言 → 准备工作台。三轮客户端对话保留前文，最后给出进入独立工作台页的入口；建图、模块操作和记忆全部留给下一整屏的真实工作台演示。后续输入不重复调用命令，中文版选择中文、英文版选择 English，仅为示例，不代替实际用户选择。

App 完整播放自动接续，到第三步停止；单章播放在本章结束后暂停。滚轮继续到下一整屏工作台，或点击对话中的工作台入口直达“逐层建图”。工作台自身的六个章节继续使用真实完成事件、原控件与 iframe，不与客户端播放器共享章节状态。减少动态时显示当前步骤完成状态，用章节按钮继续。
