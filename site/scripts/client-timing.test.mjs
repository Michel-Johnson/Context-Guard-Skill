...
test("Cursor在客户端窗口内接续工作台，其他入口仍可打开独立工作台页", () => {
  ...
  assert.match(appSource, /const pageIds = \["home", "workbench", "clients", "memory", "debug", "install", "decisions"\]/);
...
test("设计问答独立成页，并以节点关系解释 Context 管理", async () => {
  ...
  assert.ok(appSource.indexOf('<Page id="debug"') < appSource.indexOf('<Page id="install"'));
  assert.ok(appSource.indexOf('<Page id="install"') < appSource.indexOf('<Page id="decisions"'));
