...
const markerStart = source.indexOf(mapMarker);
const mapStart = markerStart + mapMarker.length;
const mapEnd = source.indexOf("const CG_OWNS = ", mapStart);
if (markerStart < 0 || mapEnd < mapStart)
  throw new Error("产品地图声明已改变，请重新核对演示接入。");
const map = JSON.parse(source.slice(mapStart, mapEnd).trim().replace(/;$/, ""));
...
const notesMarkerStart = source.indexOf(notesMarker);
...
const originalNotes = source.slice(notesStart, notesEnd).replace(/\r\n?/g, "\n");
...
await writeFile(
  ...
  JSON.stringify(
    {
      source: "prototype/workbench.html",
      sha256: digest,
      changes: [
        "隔离真实 I/O，注入内置示例文件响应",
