...
export function workbenchCamera(width: number, height: number, focus: Box | null, previous?: Camera, revealOnly = false): Camera {
  ...
  const margin = revealOnly ? 0 : Math.min(32, width * .04);
  const zoom = revealOnly ? Math.max(base, previous?.scale ?? base) : Math.max(base, Math.min(1.12, base * 1.25));
  ...
  const scale = revealOnly ? zoom : Math.max(base, Math.min(zoom, (width - margin * 2) / focus.width, (height - margin * 2) / focus.height));
