...
const RETENTION_MIGRATION_MAX_BYTES = 512 * 1024 * 1024;
...
export function compactMainHistorySnapshots(state) {
  ...
  for (const receipt of Object.values(state.receipts || {})) {
    ...
    if (expiredMain && !result.historyExpired) { result.historyExpired = true; changed = true; }
  }
  return changed;
}
...
export async function enforceMainHistoryRetention(configuration) {
  ...
  for (const projectId of Object.keys(configuration.projects || {})) {
    await withFileLock(projectMemoryLockFile(configuration.dataDir, projectId), async () => {
      ...
      const state = await readJSON(file, null, { maxReadFileBytes: RETENTION_MIGRATION_MAX_BYTES });
...
async function mergeStatus(project, sourceCommit, targetCommit) {
  try { await gitCommand(project.root, ['cat-file', '-e', `${sourceCommit}^{commit}`]); }
  catch (error) {
    if (error?.code !== 1 && !(error?.code === 128 &&
        String(error.stderr || '').includes(`Not a valid object name ${sourceCommit}^{commit}`))) throw error;
    if (!project.remote) return { ready: false, reason: 'SOURCE_COMMIT_UNAVAILABLE' };
    try { await gitCommand(project.root, ['fetch', '--quiet', project.remote, sourceCommit]); }
    catch (fetchError) {
      if (/not our ref|couldn.t find remote ref/i.test(fetchError.stderr || ''))
        return { ready: false, reason: 'SOURCE_COMMIT_UNAVAILABLE' };
      throw fetchError;
    }
  }
  try { await gitCommand(project.root, ['merge-base', '--is-ancestor', sourceCommit, targetCommit]); return { ready: true, reason: null }; }
  ...
  if (!changed.length) return { ready: true, reason: null };
  try {
  ...
  } catch (error) { if (error?.code === 1) return { ready: false, reason: 'SOURCE_NOT_MERGED' }; throw error; }
}
...
export async function commitSessionMap(configuration, projectId, sessionId, input, actor = { kind: 'human', sessionId: 'cloud-workbench' }, policy = null) {
  ...
  const committed = await withFileLock(projectMemoryLockFile(configuration.dataDir, projectId), async () => {
    ...
    state.receipts[receiptKey] = { fingerprint, result: compactReceiptResult(result), ...(policy ? { requiredGrants: operationGrants(current.memory.map, operations, grants) } : {}) };
