...
export async function startServer({ root, port = 8877, host = '127.0.0.1', fault, messageQueue = queueCodexMessage, repositoryLookup = lookupRepository } = {}) {
  ...
  async function fence(viewId, { stopping = false } = {}) {
    ...
    try {
      ...
      const deadline = Date.now() + (stopping ? Math.min(fenceDeadlineMs, 2000) : fenceDeadlineMs);
      while (pending.size) {
        ...
        if (Date.now() >= deadline) {
          // A hung SSE connection is not a live page. Evict it like a closed tab
          // so Agent reads and CLI stop are not blocked indefinitely.
          for (const peer of pending) evictPeer(peer);
          pending.clear();
          break;
        }
        await pause(15);
      }
      if (!stopping && pendingPeers(viewId).length) throw new MapError('UI_PENDING', 'A page has unsaved edits', 409, { peers: pendingPeers(viewId) });
    } finally {
    ...
    server = http.createServer(async (req, res) => {
      try {
        ...
        if (route === '/api/stop' && req.method === 'POST') {
          if (req.headers.authorization !== `Bearer ${adminToken}`) throw new MapError('UNAUTHORIZED', 'Requires local CLI credential', 401);
          for (const viewId of stores.keys()) await fence(viewId, { stopping: true });
        ...
        if (route.startsWith('/api/')) {
          ...
          if (route === '/api/presence' && req.method === 'POST') {
            ...
            if (peer) { peer.dirty = !!input.dirty; peer.version = input.version; checkpoints.get(input.checkpoint)?.delete(peer); if (input.closing && !peer.dirty) { peer.res?.end(); peers.delete(peerId); } }
