...
export function validateMemory(value) {
  ...
  validate(value.map);
  if (value.display !== undefined && (!value.display || typeof value.display !== 'object' || Array.isArray(value.display)
      || Object.keys(value.display).some(key => !['name', 'platform'].includes(key))
      || typeof value.display.name !== 'string' || !value.display.name.trim() || value.display.name.length > 200
      || typeof value.display.platform !== 'string' || value.display.platform.length > 30)) throw new MapError('INVALID_MEMORY', 'Invalid Session display metadata');
