...
async function installCoordinatorPanel(sync){
  ...
  const planning=document.createElement('p');planning.className='coordinator-planning';planning.setAttribute('aria-hidden','true');
  const planningText=document.createElement('span');planningText.className='coordinator-typing-phase';planningText.textContent='Planning next moves';planning.append(planningText);
  ...
  const updateStreamingText=(message,text,immediate=false,onDrained=null)=>{
    ...
    const revealNextChunk=()=>{
      streamTimer=0;
      const done=Boolean(streamDrained);
      const now=performance.now();
      if(now<streamNextAt){streamTimer=setTimeout(revealNextChunk,Math.max(16,streamNextAt-now));return;}
      let end=nextRevealSegmentEnd(streamTarget,streamShown.length,done);
      if(!done&&end>streamShown.length){
  ...
  historyToggle.addEventListener('click',()=>setHistoryOpen(history.hidden));
  ...
  const reviewReason=card=>new Promise(resolve=>{
    ...
    const wrap=document.createElement('div'); wrap.className='coordinator-review-inline';
    wrap.innerHTML='<h3>验收不通过</h3><label>请说明不满意的方面<textarea name="reason" maxlength="2000" placeholder="补充问题或需要修改的地方…" required></textarea></label><p>反馈会保留在当前任务中，Coordinator 将据此继续处理。</p><div><button type="submit">提交反馈</button><button type="button" data-review-cancel>取消</button></div>';
    const form=document.createElement('form'); form.append(...wrap.childNodes); wrap.replaceChildren(form);
    const textarea=wrap.querySelector('textarea'); let settled=false;
    const finish=value=>{if(settled)return; settled=true; wrap.remove(); resolve(value);};
    form.addEventListener('submit',event=>{event.preventDefault(); const value=textarea.value.trim(); if(!value){textarea.focus();return;} finish(value);});
  ...
  const messageMatchesRequest=(message,request)=>{
    ...
    if(message.requestId)return message.requestId===request.id;
  ...
  let creatingConversation=false,newConversationRequest=null;
  creationToggle.addEventListener('click',async()=>{
    if(creatingConversation)return;
    creatingConversation=true;creationToggle.disabled=true;setHistoryOpen(false);status.textContent='正在新建 Coordinator Session…';
    newConversationRequest||={id:crypto.randomUUID()};
    try{
      const result=await sync.call('/api/coordinator/conversations/new',newConversationRequest,'POST','main');
      newConversationRequest=null;status.textContent='';selectConversation(result.id);
    }catch(error){status.textContent='新建 Coordinator Session 尚未确认：'+error.message+'；再次点击 ＋ 重试。';}
    finally{creatingConversation=false;creationToggle.disabled=false;}
  });
  ...
  const render=(state,forceFinal=false)=>{
    ...
    }else if(!hasStreaming) stopStreamingAnimation();
    ...
    for(const acceptance of state.acceptances||[]){
      ...
      for(const [decision,label] of [['approved','验收通过'],['rejected','验收不通过']]){
        ...
        button.addEventListener('click',async()=>{
          if(!request){
            const reason=decision==='rejected'?await reviewReason(card):label;
