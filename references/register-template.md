...
Use this format for `.codex/context/bad-cases.md` or the existing project context register.
...
- Guard / verification: automated test, reusable script command, manual check, screenshot, log, or invariant
- Reusable guard path: project test file, `.codex/context/bad-case-tests/...`, or none
- Guard reuse rule: run this existing guard before creating any new test for this case
...
## Reusable Guard Rules

- Prefer an existing recorded guard command over a newly invented test.
- Store bad-case-specific guard scripts under `.codex/context/bad-case-tests/` when they do not belong in the native test suite.
- Use `.codex/context/` as the project folder for bad-case memory. Do not introduce a separate bad-case folder for new projects.
- Name guard scripts with the bad case ID so they are easy to find and reuse.
- Update an existing guard when the expected behavior changes; do not create parallel scripts for the same case unless the old one is explicitly obsolete.
- If a guard is manual-only, explain why automation is not practical and list the exact manual check.
