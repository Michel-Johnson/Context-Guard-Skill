...
普通开发 Agent 写入自己的 Session Map，不是 Main。Cloud Coordinator 若项目配置显式启用 `mapWrite`，可以通过服务端工具直接创建、改名、更新、移动或删除 Main 节点，也可以删除指定节点上的 TODO/Bug，并以 `coordinator` 身份审计；版本校验、幂等回执和根节点保护仍由服务端强制执行。命令细节以 [workbench-interface.md](workbench-interface.md) 为准。
...
普通开发 Agent 需要新职责时用 `map apply` 提交 create，带上 `owns` 与 `proposalEvidence`。Coordinator 使用注入目录中的稳定父节点 ID、记录所属 `nodeId` 与当前 Main 版本调用 `edit_map`；页面收到提交事件后负责渲染节点和动效。两者都不能直接改 `map.json`，冲突后先重读权威 Main，再按原操作身份核对回执。
