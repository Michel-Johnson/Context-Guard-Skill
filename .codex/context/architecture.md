...
- `visibleChildren`：根目录只铺 L1。模块一次只画一层，不展开子模块。开工节点若后面还有开工节点（最多两岔）则默认展开；多于两个分支的开工节点晋升为模块。提议的模块和开工节点右上角绿点 = Agent 生成、尚未人批。提议模块不挂锁。
- 根和「孩子全是子模块」的视图用网格目录；左右/上下只排模块内部的开工树
- 检查器只留标题、两行用途、必要按钮。记忆/Bug 是区块名，不可改。开工节点默认不贴「探索中」芯片（那是 dirty 缺省，不是活状态）；有成功/失败/自定义标签才显示。Bug 只显示标题。提议节点更短：标题、用途、加入/隐藏。
