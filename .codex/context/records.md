...
## 修 bug 时顺着哪条线走（讨论稿，还没做对照）

人的假设：卡和卡之间是能互相找到的指针；有的联系不必分谁指向谁。Agent 修一条坏例时，先读这条链上的规矩；碰到别的模块，再顺着关系去拿。

现在实际是：

- 上下级树：一直都有。点未修 Bug，工作台只画出从根到那张卡的链，不把有关系的模块拉进来。
- 关系线：地图里存的是「谁产出给谁」，带方向和一句说明。人打开「关系」时，点一张卡会高亮两边的对端——给人看时已经当成能往返。Agent 改文件 / 修 bug **还不会**顺着这条线去读记忆。
- 所以缺的不是再画一种线，而是约定 **什么时候才去隔壁那张卡**。

几种底层，用修 bug 来想：

1. **只走上下级。** 先读这张卡和它上面的模块。简单，不容易一次读太多。隔壁其实在给它送东西时，你会漏。
2. **上下级 + 有方向的产出/消费（现在存的）。** 人能看懂「谁给谁」。若 Agent 只许顺着箭头走，站在接收的那头会找不到送东西的那头。
3. **上下级 + 不分方向的邻居。** 有关系就算邻居。先读这条链，需要时再读旁边一圈。接近「没有方向的指针」。关系一密，一圈就会变大。
4. **每对相关卡都存来回两根针。** 和 3 几乎一样，查起来省事，改的时候容易漏一边。
5. **先链后邻居（更像你说的加载顺序）。** 存法仍是树 + 关系；读的时候先链上的短规矩，只有当前这件事牵到另一张卡（换了文件，或关系上的对端）才打开那张。不要一修 bug 就把所有相关模块灌进来。

倾向先按 5 来讨论：关系线给人看时可以保留「谁产出给谁」这句话；Agent 加载时把对端当成能往返的邻居，而且只走一跳。下一步若做对照，就在 Harbor 上比三种读法——只读链、链加一圈邻居、链加只顺着箭头——看会不会漏、会不会一次读太多。现在先不定死。
