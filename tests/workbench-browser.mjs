...
try {
  ...
  await page.locator('.node[data-id="N1"]').click();
  await page.locator('#detail [data-act="add-bug"]').click();
  const failedEditor = page.locator('#detail [data-ed="bug-title"]').last();
  await failedEditor.fill('发送失败测试');
  await failedEditor.press('Tab');
  await page.waitForFunction(() => document.querySelector('#bug-panel-list li')?.textContent?.includes('待处理 · 发送失败'));
  await until(async () => (await read()).root.children[0].bugs.find(bug => bug.title === '发送失败测试')?.dispatch?.status === 'failed');
  await synchronized();
  const failedState = await read();
  const failedBug = failedState.root.children[0].bugs.find(bug => bug.title === '发送失败测试');
