...
ROOT_FAIL="$(mktemp -d "${TMPDIR:-/tmp}/context-guard-feature-chain-checkpoint-fail-XXXXXX")"
ROOT_PASS="$(mktemp -d "${TMPDIR:-/tmp}/context-guard-feature-chain-checkpoint-pass-XXXXXX")"
trap 'rm -rf "$ROOT_FAIL" "$ROOT_PASS"' EXIT
OUT="$ROOT_FAIL/out.txt"

python3 "$SCRIPT" init --root "$ROOT_FAIL" >/dev/null
...
python3 - "$ROOT_FAIL/.codex/context/test-hub/feature-chains.json" "$CHAIN_ID" <<'PY'
...
grep -q "checkpoint failed: 前端打开监控页 - 缺少 grafana_url" "$OUT"
...
assert result["reason"] == "checkpoint failed: 前端打开监控页 - 缺少 grafana_url"
assert [item["status"] for item in result["checkpoints"]] == ["pass", "fail"]
PY

OUT="$ROOT_PASS/out.txt"
python3 "$SCRIPT" init --root "$ROOT_PASS" >/dev/null
ADD_OUTPUT="$(python3 "$SCRIPT" feature-chain-add \
  --root "$ROOT_PASS" \
  --title "GPU 监控按钮" \
  --entry "点击 GPU 监控按钮" \
...
python3 - "$ROOT_PASS/.codex/context/test-hub/last-run.json" <<'PY'
from pathlib import Path
import json
import sys
...
assert result["checkpoints"][0]["label"] == "后端返回监控 URL"
PY
