...
Human–agent project memory. The agent records. The human looks at the HTML workbench and confirms there (local write-back) or in chat after a GitHack view (cloud). Not the CLI.
...
**Local (browser and repo on the same machine):** serve the repo root and open `prototype/workbench.html`, or use the workbench 「连接仓库」 control so the browser writes `map.json` on disk. Next turn, re-read the small indexes. Do not reuse a map pasted in an earlier message.
