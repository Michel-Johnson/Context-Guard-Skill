...
4. The HTML map shows only the first layer as unconfirmed proposals — that layer is a confirmation gate, not the entire analysis. Deeper, development-grain nodes must already exist in `architecture.md` and in each module's inbox. Later agents must not read unconfirmed proposals.
5. The human confirms, rewrites, or hides L1 modules. After a module is confirmed, unpack the next already-analyzed layer when they enter it or ask to expand. Do not wait until they enter to *start* thinking; the depth should already be in the notes.
