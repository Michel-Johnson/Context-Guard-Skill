...
4. **owns vs files vs record.** `owns` is source ownership: repo-relative files, or directories ending in `/`. `files` is evidence (shots, logs), not ownership. `record` on a bug points at the index card `.codex/context/bugs/{id}.md`; how-to-fix lives in `.codex/context/fixes/{id}.md`. One source path has one owner. When looking up many paths/bugs/tasks at once, run one `python3 scripts/map_owns.py jump --json '{"path":[...],"bug":[...],"task":[...],"last":true}'` and open only `open` (then `then` if needed). Do not spawn `jump` N times. Do not paste `jump-index.json` or `map.json` into the turn. A single lookup may use `--path|--bug|--task|--last`. Do not rely on Markdown hyperlinks. If a path has no owner, do not invent one. After the map changes, run `python3 scripts/map_owns.py cards`.
