...
4. **owns vs files vs record.** `owns` is source ownership: repo-relative files, or directories ending in `/`. `files` is evidence (shots, logs), not ownership. `record` on a bug points at the index card `.codex/context/bugs/{id}.md`; how-to-fix lives in `.codex/context/fixes/{id}.md`. One source path has one owner. When the agent edits a file, reports a bug, redoes a task, or asks what happened last session, run `python3 scripts/map_owns.py jump --path|--bug|--task|--last` and open only the `open` files (then `then` if needed). Do not paste the whole `map.json`. Do not rely on Markdown hyperlinks. If a path has no owner, do not invent one. After the map changes, run `python3 scripts/map_owns.py cards`.
