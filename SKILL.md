...
4. **owns vs files vs record.** `owns` is source ownership: repo-relative files, or directories ending in `/`. `files` is evidence (shots, logs), not ownership. `record` on a bug points at the index card `.codex/context/bugs/{id}.md`; how-to-fix lives in `.codex/context/fixes/{id}.md`. One source path has one owner. When looking up many paths/bugs/tasks at once, read `.codex/context/jump-index.json` once and open only the hit Markdown files — do not spawn `jump` N times. If a tool is still needed, run one `python3 scripts/map_owns.py jump --json '{"path":[...],"bug":[...],"task":[...],"last":true}'` and open only `open` (then `then` if needed). A single lookup may use `--path|--bug|--task|--last`. Do not paste the whole `map.json`. Do not rely on Markdown hyperlinks. If a path has no owner, do not invent one. After the map changes, run `python3 scripts/map_owns.py cards`.
