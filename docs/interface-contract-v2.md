...
心跳 Session 可附 `execution:{status:"active"|"stopped"|"interrupted"|"failed"|"unknown",at:string}`，由本地宿主状态提供，不依赖 Hook。`at` 是宿主观察时间，不用于延长在线期限；在线仍以服务器接收时间为准。Cloud access 返回此执行状态，离线时返回 unknown；连接在线、宿主运行和业务任务完成是三个不同事实，心跳不能代替任务完成回执。
